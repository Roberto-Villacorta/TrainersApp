from bbdd.models import Atleta, Suscripcion, Llamada
...
class AtletasService:
    ...
    def actualizar_atleta(self, atleta_id, **kwargs):
        ...
        nombre_antiguo = atleta.nombre_completo
        
        for key, value in kwargs.items():
            if hasattr(atleta, key):
                setattr(atleta, key, value)
                
        # Si el nombre cambió, actualizar todas sus llamadas programadas
        if "nombre_completo" in kwargs and kwargs["nombre_completo"] != nombre_antiguo:
            self.session.query(Llamada).filter(Llamada.nombre == nombre_antiguo).update({"nombre": kwargs["nombre_completo"]})
    ...
    def cambiar_estado(self, atleta_id, nuevo_estado="inactivo", fecha_comienzo=None):
        ...
        if atleta:
            ...
            if nuevo_estado == "activo":
                ...
                # Si se desactiva, borrar sus llamadas pendientes
                self.session.query(Llamada).filter(Llamada.nombre == atleta.nombre_completo).delete()
            self.session.commit()
            return True
        return False
